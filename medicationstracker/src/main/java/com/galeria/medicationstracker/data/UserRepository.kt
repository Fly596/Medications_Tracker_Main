...
constructor(private val firestore: FirebaseFirestore, private val auth: FirebaseAuth) :
  ...
  override suspend fun getUserData(): Result<User> {
    return try {
      val snapshot =
        firestore.collection("User").document(auth.currentUser?.email.toString()).get().await()
      val user = snapshot.toObject(User::class.java)

      if (user != null) Result.success(user)
      else Result.failure(Exception("Юзер не найден, лошара"))
    } catch (e: Exception) {
      Result.failure(e)
    }
  }
  ...
  override suspend fun updateUserData(user: User) {
    firestore.collection("User").document(auth.currentUser?.email.toString()).set(user)
  ...
  override suspend fun getUserDrugs(): List<UserMedication> {
    return try {
      val querySnapshot =
        db.collection("UserMedication").whereEqualTo("uid", auth.currentUser?.uid).get().await()
