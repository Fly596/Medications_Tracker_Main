...
class NewMoodRepositoryImpl @Inject constructor(private val firestore: FirebaseFirestore) :
    ...
    override fun observeUserMoods(userId: String): Flow<List<NetworkUserMood>> =
        callbackFlow {
            val listenerRegistration =
                firestore
                    ...
                    .addSnapshotListener { snapshot, error ->
                        ...
                        if (snapshot != null) {
                            val moods =
                                snapshot.toObjects(NetworkUserMood::class.java)
    ...
    override suspend fun getMood(
        userId: String,
        moodId: String
    ): Result<NetworkUserMood> {
        return try {
            ...
            if (documentSnapshot.exists()) {
                val mood =
                    documentSnapshot.toObject(NetworkUserMood::class.java)
                if (mood != null) {
                    Result.success(mood)
                } else {
