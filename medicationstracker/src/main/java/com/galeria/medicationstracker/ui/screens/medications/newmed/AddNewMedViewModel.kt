...
constructor(
    ...
    fun addMedication(context: Context) {
        viewModelScope.launch {
            // Проверка на пустые значения текстовых полей и нулевое значение
            // medStrength
            // ? в репо
            if (uiState.value.medName.isBlank() ||
                uiState.value.medForm.isBlank() ||
                ...
                medicationRepository.addMedicationOld(networkMedication)
            }
        }
    }
    ...
    fun addStrength(newStrength: Float) {
        uiState.value =
            uiState.value.copy(
                chosenStrengths = uiState.value.chosenStrengths + newStrength)
    }
